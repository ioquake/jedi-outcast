	...
	switch(cinTable[currentHandle].roq_id) 
	{
		...
		case	ROQ_QUAD_INFO:
			if (cinTable[currentHandle].numQuads == -1) {
				...
				setupQuad( 0, 0 );
				cinTable[currentHandle].startTime = cinTable[currentHandle].lastTime = Com_Milliseconds()*com_timescale->value;
			}
...
e_status CIN_RunCinematic (int handle)
{
	...
	thisTime = Com_Milliseconds()*com_timescale->value;
	if (cinTable[currentHandle].shader && (abs(thisTime - cinTable[currentHandle].lastTime))>100) {
		cinTable[currentHandle].startTime += thisTime - cinTable[currentHandle].lastTime;
	}
	cinTable[currentHandle].tfps = ((((Com_Milliseconds()*com_timescale->value) - cinTable[currentHandle].startTime)*cinTable[currentHandle].roqFPS)/1000);
		...
		&& (cinTable[currentHandle].status == FMV_PLAY) ) 
	{
		...
		if (start != cinTable[currentHandle].startTime) {
		  cinTable[currentHandle].tfps = ((((Com_Milliseconds()*com_timescale->value)
...
static void PlayCinematic(const char *arg, const char *s, qboolean qbInGame)
{
	...
	if (S_FileExists( arg ))
	{
		...
		qboolean	bIsForeign	= s_language && stricmp(s_language->string,"english");
